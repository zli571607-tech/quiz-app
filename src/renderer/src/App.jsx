    ...
    if (this.state.hasError) return (
      <div className="p-8 flex items-center justify-center min-h-full">
        <div className="max-w-md text-center">
          <AlertTriangle size={48} className="text-wrong mx-auto mb-4" />
          <h2 className="text-lg font-semibold mb-2">页面出错</h2>
          <p className="text-sm text-text-secondary mb-4">{this.state.error?.message || '未知错误'}</p>
          <button onClick={() => { this.setState({ hasError: false }); window.location.reload() }}
            className="bg-primary text-white px-4 py-2 rounded-btn text-sm hover:bg-primary-hover">刷新重试</button>
        </div>
      </div>
    )
...
function Sidebar({ mobileOpen, onClose }) {
  ...
  const content = (
    <>
      <div className="px-5 py-6 border-b border-border flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-primary">📝 刷题软件</h1>
          <p className="text-xs text-text-secondary mt-1">{dbReady ? 'AI 驱动的学习工具' : '⏳ 加载中...'}</p>
        </div>
        {/* 移动端关闭按钮 */}
          ...
          return (
            <NavLink key={path} to={path} onClick={onClose}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-btn text-sm font-medium transition-colors
                ${isActive ? 'bg-primary text-white shadow-sm' : 'text-text-secondary hover:bg-white hover:text-text-primary'}`}>
              <Icon size={18} />{label}
            </NavLink>
          )
        })}
  ...
  return (
    <>
      {/* 桌面端固定侧边栏 */}
      <aside className="hidden md:flex w-56 bg-sidebar min-h-screen border-r border-border flex-col shrink-0">{content}</aside>
      {/* 移动端覆盖层 */}
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          <div className="fixed inset-0 bg-black/50" onClick={onClose} />
          <aside className="relative w-64 bg-sidebar min-h-full border-r border-border flex flex-col z-50 animate-slide-in">{content}</aside>
        </div>
      )}
    </>
  )
...
function HomePage() {
  ...
  if (dbError) return (
    <div className="p-8 flex items-center justify-center min-h-full">
      <div className="max-w-md text-center">
        <AlertTriangle size={48} className="text-wrong mx-auto mb-4" />
        <h2 className="text-lg font-semibold mb-2">数据库加载失败</h2>
        <p className="text-sm text-text-secondary mb-4">{dbError}</p>
        <button onClick={() => window.location.reload()} className="bg-primary text-white px-4 py-2 rounded-btn text-sm">刷新页面</button>
      </div>
    </div>
  )
  ...
  return (
    <div className="p-4 md:p-8">
      <h2 className="text-xl md:text-2xl font-semibold mb-4 md:mb-6">欢迎回来 👋</h2>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-6 md:mb-8">
        {[
          { label: '题库总数', value: stats.totalQuestions, unit: '题' },
          { label: '累计刷题', value: stats.totalSessions, unit: '次' },
          { label: '平均正确率', value: stats.totalAnswers > 0 ? stats.accuracy : '--', unit: '%' },
        ].map(({ label, value, unit }) => (
          <div key={label} className="bg-card rounded-card p-4 md:p-5 border border-border">
            <p className="text-xs md:text-sm text-text-secondary">{label}</p>
            <p className="text-2xl md:text-3xl font-semibold mt-2">{value}<span className="text-base md:text-lg text-text-secondary ml-1">{unit}</span></p>
          </div>
        ))}
      </div>
